...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32* new_page_table = kmalloc(4*1024);

	if(new_page_table==NULL)
	{
		cprintf("There is No memory Exist");
	}

	ptr_page_directory[PDX(virtual_address)] = CONSTRUCT_ENTRY((uint32)(kheap_physical_address((unsigned int )new_page_table)), PERM_USER|PERM_WRITEABLE|PERM_PRESENT);

	//access all its entry and assign it with 0
	for(int entry=0;entry<1024;entry++)
	{
		new_page_table[entry]=0;
	}

	//to make sure there is no garbage
	tlbflush();
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	{
		int returned_Result = pf_add_empty_env_page(e,virtual_address,1);

		if(returned_Result == E_NO_PAGE_FILE_SPACE)
		{
			panic("No space");
		}

		virtual_address += (4*1024);
...
void remove_env(struct Env* removedEnv, uint32 virtAddress, uint32 pages)
{
	for(int i=0; i<pages;i++)
	{
		pf_remove_env_page(removedEnv,virtAddress);
		virtAddress +=PAGE_SIZE;
	}
}

void removeWSpages(struct Env* removedWS, uint32 virtAddress, uint32 pagesSize)
{

	uint32 maximumSize = removedWS->page_WS_max_size;

	for(int i = 0;i < maximumSize; i++)
	{
		uint32 WSvirt_Address = env_page_ws_get_virtual_address(removedWS,i);
		uint32 addVirtToPS = virtAddress+ROUNDUP(pagesSize, PAGE_SIZE);
		if(WSvirt_Address >= virtAddress)
		{
			if(WSvirt_Address < addVirtToPS)
			{
				unmap_frame(removedWS->env_page_directory, (void*)WSvirt_Address);
				env_page_ws_clear_entry(removedWS, i);
...
void removePages(struct Env* removeEnv, uint32 virtAddress,uint32 numOfPages)
{
	uint32* page_table;
	...
	for(int i = 0; i<numOfPages; i++)
	{
		...
		page_table=NULL;
		get_page_table(removeEnv->env_page_directory,(void*)virtAddress,&page_table);
		...
		if(page_table !=NULL)
		{
			...
			if(bool > 0)
			{
				...
				pd_clear_page_dir_entry(removeEnv,(uint32)virtAddress);
			}
		}
		virtAddress+=(4*1024);
	}
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 saveVirtualAddress = virtual_address;
	remove_env(e, saveVirtualAddress,  pagesNumber);

	saveVirtualAddress=virtual_address;
	removeWSpages(e, saveVirtualAddress, size);

	saveVirtualAddress= virtual_address;
	removePages(e, saveVirtualAddress,pagesNumber);
