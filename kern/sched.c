...
void sched_init_MLFQ(uint8 numOfLevels, uint8 *quantumOfEachLevel)
{
	...
	env_ready_queues = kmalloc(num_of_ready_queues*sizeof(env_ready_queues));

	for(int queue = 0; queue<num_of_ready_queues; queue++)
	{
	...
	for(int quantum = 0; quantum < numOfLevels; quantum++)
	{
		quantums[quantum] = quantumOfEachLevel[quantum];
	}
...
struct Env* fos_scheduler_MLFQ()
{
	...
	if (curenv != NULL)
	{
		enqueue(&env_ready_queues[queueCounter], curenv);
	}
	...
	for (int next = 0; next < num_of_ready_queues; next++)
	{

		int sizeOfQueue = queue_size(&env_ready_queues[next]);

		if (sizeOfQueue > 0)
		{
			//get the next in queue
			struct Env* nextEnv = dequeue(&env_ready_queues[next]);
			//set it with the next value
			kclock_set_quantum(quantums[next]);

			if (next < num_of_ready_queues - 1)
			{
				queueCounter = next + 1;
			}
			else
			{
				//if the queue reach the maximum
				queueCounter = num_of_ready_queues - 1;
			}
