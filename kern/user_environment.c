...
void env_free(struct Env *e)
{
	...
	uint32 pagesWS = e->page_WS_max_size;

	for(int page=0;page<pagesWS;page++)
	{
		if(!env_page_ws_is_entry_empty(e,page))
		{
			unmap_frame(e->env_page_directory,(void*)env_page_ws_get_virtual_address(e,page));
		}
	}
	...
	for(int i = 0; i < PDX(USER_TOP); i++)
	{
		...
		{
			uint32 getPhysicalAddress = (e->env_page_directory[i] & 0xFFFFF000);
			struct Frame_Info* frame_info = to_frame_info(getPhysicalAddress);

			if (frame_info->references != 0)
			{
				frame_info->references = 0;
