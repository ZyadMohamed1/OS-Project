...
void calcLastIndex(struct Env * env) {
	int counter = 0;
	while (counter < env->page_WS_max_size) {
		if (env->ptr_pageWorkingSet[env->page_last_WS_index].empty)
			break;
		if (env->ptr_pageWorkingSet[counter].empty) {
			env->page_last_WS_index = counter;
			break;
		}
		counter++;
	}
...
//Insert in the entry
void insertEntry(struct Env * env, uint32 fault_va) {
	struct Frame_Info *frame_info_ptr = NULL;
	int result = allocate_frame(&frame_info_ptr);
	if (result == E_NO_MEM)
		panic("page_fault_handler\n");
	else {

		map_frame(env->env_page_directory, frame_info_ptr, (void*) fault_va,
		PERM_PRESENT | PERM_USER | PERM_WRITEABLE);
		result = pf_read_env_page(env, (void *) fault_va);
		if (result == E_PAGE_NOT_EXIST_IN_PF) {
			if (fault_va < USTACKTOP && fault_va >= USTACKBOTTOM)
				pf_add_empty_env_page(env, fault_va, 0);
			else {
				unmap_frame(env->env_page_directory, (void *) fault_va);
				panic("Can't Access Address --> %x", fault_va);
			}
			calcLastIndex(env);
			env_page_ws_set_entry(env, env->page_last_WS_index, fault_va);
			env->page_last_WS_index++;
			env->page_last_WS_index = env->page_last_WS_index
...
//Check if the address is used or not
int checkUsedDirtyBit(struct Env * curenv, uint32 fault_va) {
	uint32 *ENVPTpointer = NULL;
	uint32 *VAPTpointer = NULL;
	for (int WSCounter = 0; WSCounter < curenv->page_WS_max_size; WSCounter++) {

		uint32 victAddress = env_page_ws_get_virtual_address(curenv,
				curenv->page_last_WS_index);
		struct Frame_Info* ptr_frame_info1 = get_frame_info(
				curenv->env_page_directory, (void*) victAddress, &ENVPTpointer);
		get_page_table(curenv->env_page_directory, (void *) victAddress,
				&VAPTpointer);
		if (((VAPTpointer[PTX(victAddress)] & PERM_USED) == 0)
				&& ((VAPTpointer[PTX(victAddress)] & PERM_MODIFIED) == 0)) {
			pf_update_env_page(curenv, (void *) victAddress, ptr_frame_info1);
			unmap_frame(curenv->env_page_directory, (void*) victAddress);
			insertEntry(curenv, fault_va);
			return 1;
		}
		curenv->page_last_WS_index++;
		curenv->page_last_WS_index = curenv->page_last_WS_index
...
// Check the used bit
int checkUsedBit(struct Env * curenv, uint32 fault_va) {
	uint32 *ENVPTpointer = NULL;
	uint32 *VAPTpointer = NULL;
	for (int WSCounter = 0; WSCounter < curenv->page_WS_max_size; WSCounter++) {
		...
		if ((VAPTpointer[PTX(victAddress)] & PERM_USED) == 0) {
			...
			VAPTpointer[PTX(victAddress)] &= (~PERM_USED);
...
void page_fault_handler(struct Env * curenv, uint32 fault_va) {
	...
	//Reinsert page
		...
		if (checkUsedDirtyBit(curenv, fault_va) == 0) {
			if (checkUsedBit(curenv, fault_va) == 0)
				checkUsedDirtyBit(curenv, fault_va);
		}
	}
}
